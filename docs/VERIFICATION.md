...
- 為保留全部 16,661 點，本次接受既有 eager 軌跡檔增加；發布預算鎖定原始檔不超過 4.4 MB（涵蓋 Windows CRLF checkout）、gzip 不超過 525 KB。390px 系統 Chrome 新頁面載入至首頁主標三次為 3,154／533／612 ms（首次含冷啟動），互動檢查通過。後續若再加入大型軌跡，應先改為按需載入。
