...
## 2026-08-31 GPS Art 三條站內軌跡與分頁

- 正式 production catalog 為 25 件：5 件 `track-ready`、0 件 `source-download`、20 件 `source-only`；活動分布為 21 件自行車與 4 件跑步／步行。
- 新增河濱海馬、野雁西飛、汐鴿三件站內軌跡。河濱海馬與汐鴿使用匿名公開 GPX；野雁西飛依公開行程節點與參考圖，以 BRouter `fastbike` 重建道路軌跡，不冒稱為原 Strava 座標。
- 五種篩選的實際數量為全部 25、自行車 21、跑步／步行 4、可下載 GPX 5、站內地圖 5；篩選結果每頁 12 件，切換篩選會回到第一頁，頁碼會依結果數限制。
- 站內地圖與下載 GPX 共用同一份凍結 `segments`；來源檔、canonical geometry SHA-256、逐段點數、總點數與台灣 bounds 由測試鎖定。正式路線仍為 23 個 bundle／68 條路線，GPS Art 不進入 manifest。
- 現行 `npm run verify` 基準為 396 項測試；來源與重建界線記錄於 `docs/route-research/taiwan-gps-art.md`。

## 歷史快照：2026-08-28 GPS Art 來源下載現況

> 本節保留當時 22 件／2 件站內軌跡的匿名下載查核，不代表 2026-08-31 現行數量。

- 當時正式 production catalog 維持 22 件，三狀態實際分布為 2 件 `track-ready`、0 件 `source-download`、20 件 `source-only`；不以原訂 41 件目標取代現場驗證結果。
...
- 當時 `npm run verify` 基準為 389 項測試；來源矩陣、權利界線、瀏覽器證據與未驗證邊界同步記錄於 `docs/route-research/taiwan-gps-art.md`、`docs/TEST-PLAN.md` 與 `docs/VERIFICATION.md`。

## 歷史快照：2026-08-14 GPS Art 圖鑑增補

> 本節保留 2026-08-14 的四篩選與來源查核紀錄，不代表 2026-08-31 現行介面或測試數字；現況以上方現行章節為準。
...
- 8 項經典挑戰與 25 件獨立 GPS Art 公開來源圖鑑；現況為 5 件 `track-ready`、0 件 `source-download`、20 件 `source-only`。
...
│  ├─ data/route-art-catalog.js  # 25 件 GPS Art 三狀態公開來源圖鑑
│  ├─ data/route-art-tracks.js   # 五件 KML／GPX／BRouter 凍結分段軌跡
...
- 發布前必須掃描 BRouter raw waytags 並 hard-fail 明確禁行、渡輪、階梯、高速公路及未證明可供公路車騎乘的條件式道路；例外只能以具距離上限、理由與 HTTPS 來源的 seed 結構宣告。
- 正式 bundle 必須保存原始 BRouter 幾何與道路稽核 SHA-256，並由受版控的 `tools/route-data/road-audit.json` 在 CI 完整重建軌跡及重算政策結果；道路例外另綁定精確路段 SHA-256。
...
> 本節的 317 項測試是百科里程碑當日證據；2026-08-31 現行基準為 396 項。
...
- [x] 文件同步與 fresh 317 項 `npm run verify`、`git diff --check`、敏感資訊掃描結果記錄於 `docs/VERIFICATION.md`。
...
- [x] 四主題可切換並持久化。
- [x] 68 條正式路線可搜尋、篩選與檢視。
- [x] 路線美學顯示 25 件公開來源作品，5 件站內軌跡可顯示並輸出同源 GPX；0 件 `source-download`，20 件 `source-only` 不建立假幾何；結果每頁 12 件。
- [x] 每條路線可下載有效 GPX。
