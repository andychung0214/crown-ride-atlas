...
test("圖鑑固定收錄 30 件有公開來源的台灣 GPS Art", () => {
  assert.equal(Catalog.length, 30);
  assert.equal(new Set(Catalog.map(item => item.id)).size, 30);
...
test("catalog 對單件必要軌跡缺失或 segments 無效採逐件降級", () => {
  ...
  for (const scenario of cases) {
    ...
    assert.equal(catalog.length, 30, scenario.name);
...
test("正式圖鑑的三種狀態反映目前驗證結果", () => {
  assert.deepEqual(RouteArt.stats(Catalog), {
    total: 30, trackReady: 10, sourceDownload: 0, sourceOnly: 20
  });
  assert.equal(Data.routes.length, 69);
  assert.equal(new Set(Object.values(TrackManifest).map(entry => entry.bundleId)).size, 23);
});

test("下載摘要模組缺失時保留 30 件，存在時才合併並驗證每一筆", () => {
  ...
  assert.equal(withoutDownloads.length, 30);
  assert.equal(withDownload.length, 31);
...
test("Node 載入器只對不存在的下載摘要 fail-soft，malformed 摘要照常失敗", () => {
  const validDownload = sourceDownloadFixture();
  assert.equal(loadNodeCatalog(undefined, { downloadsPresent: false }).length, 30);
  assert.equal(loadNodeCatalog({ [validDownload.id]: validDownload }).length, 31);
...
test("Node 只在精確下載摘要目標不存在時 fail-soft", t => {
  ...
  assert.equal(require(missingTargetCatalogPath).length, 30);
